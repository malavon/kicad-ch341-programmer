...
render () {
	kicad-cli pcb render -o ${DIR_EXPORT}/render/$1.png --preset pcb -w ${PNG_W} -h ${PNG_H} --side $1 --quality basic \
	--zoom ${4:-1} --rotate "${2:-0,0,0}" --pan "${3:-0,0,0}" "${PCB}" > /dev/null
}

renderIso() {
	kicad-cli pcb render -o ${DIR_EXPORT}/render/iso-$1.png --preset populated --background opaque -w ${PNG_W} -h ${PNG_H} --side $1 --quality high --perspective \
	--zoom ${4:-1} --rotate "${2:-0,0,0}" --pan "${3:-0,0,0}" "${PCB}" > /dev/null
}

exportGerbers() {
	local mfr=$1
	local zip="gerbers-${mfr}.zip"
	kicad-cli pcb export gerbers -o ${DIR_EXPORT}/gerbers -l ${LAYERS_ALL} -D MFR=${mfr} --check-zones --ev --hdnp --subtract-soldermask "${PCB}" # 
	# gerbers are zipped separately for ease-of-use
	(rm ${zip}; cd "${DIR_EXPORT}/gerbers" && zip -r ../${zip} *)
}

exportManufacturables() {
	kicad-cli pcb export drill   -o ${DIR_EXPORT}/gerbers --excellon-separate-th "${PCB}" # --map-format gerberx2 
	
	exportGerbers elecrow
	exportGerbers jlcpcb
	exportGerbers multicb # most importantly
	
	kicad-cli sch export bom     -o ${DIR_EXPORT}/bom.tsv --format-preset TSV --exclude-dnp \
			--fields "QUANTITY,Reference,Value,Description,Mfr,Part,AltMfr1,AltPart1" \
			--labels "Qty,Refs,Value,Description,Manufacturer,Part #,Alt. Mfr 1,Alt.Part #1" \
			--group-by "Value,Mfr,Part" "${SCH}"
}
...
exportPcbViews() {
	...
	kicad-cli pcb export svg -o "${base}-sheet-top.svg"   -l ${LAYERS_TOP} ${svgArgs} "${PCB}"
	kicad-cli pcb export svg -o "${base}-sheet-btm.svg"   -l ${LAYERS_BTM} ${svgArgs} "${PCB}"
	...
	kicad-cli pcb export pdf -o "${base}-sheet.pdf"   -l ${LAYERS_ALL} ${pdfArgs} --include-border-title "${PCB}"
